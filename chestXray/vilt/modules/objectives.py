...
def cosine_similarity_loss(tensor_a, tensor_b, tensor_c):
    # Normalize tensors
    tensor_a = F.normalize(tensor_a, p=2, dim=0)
    tensor_b = F.normalize(tensor_b, p=2, dim=0)
    tensor_c = F.normalize(tensor_c, p=2, dim=0)
    # Compute cosine similarity for each pair
    cos_sim_ab = torch.sum(tensor_a * tensor_b, dim=0)
    cos_sim_bc = torch.sum(tensor_b * tensor_c, dim=0)
    cos_sim_ca = torch.sum(tensor_c * tensor_a, dim=0)

    # Loss: average of (1 - cosine similarity) for all pairs
    loss = (1 - cos_sim_ab).mean() + (1 - cos_sim_bc).mean() + (1 - cos_sim_ca).mean()


    return loss / 3  # Average loss over the three pairs


def compute_chestXray(pl_module, batch):
    phase = "train" if pl_module.training else "val"
    if phase == "train":
        infer = pl_module.infer(batch, mask_text=False, mask_image=False)
    else:
    ...
    imgcls_logits = pl_module.chestXray_classifier(infer["cls_feats"])
    ...
    ret = {
        "chestXray_loss": imgcls_loss,
        "chestXray_logits": imgcls_logits,
        "chestXray_labels": imgcls_labels,
    }

    loss = getattr(pl_module, f"{phase}_chestXray_loss")(ret["chestXray_loss"])
    
    f1_scores = getattr(pl_module, f"{phase}_chestXray_F1_scores")(
        ret["chestXray_logits"], ret["chestXray_labels"]
    )
    pl_module.log(f"chestXray/{phase}/loss", loss)
