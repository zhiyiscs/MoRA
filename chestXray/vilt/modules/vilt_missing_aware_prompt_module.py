...
from vilt.modules.lora import LoRALinearLayer,LoRAConv1dLayer,LoRAMissingLayer
...
class ViLTransformerSS(pl.LightningModule):
    def __init__(self, config):
        ...
        # self.pooler = heads.Prompt_Pooler(len(self.hparams.config["prompt_layers"]),self.hparams.config["prompt_length"],config["hidden_size"])
        ...
        ##################################
        if self.hparams.config["loss_names"]["chestXray"] > 0:
            cls_num = self.hparams.config["chestXray_class_num"]
            self.chestXray_classifier = nn.Sequential(
                nn.Linear(hs, hs * 2),
                nn.LayerNorm(hs * 2),
                nn.GELU(),
                nn.Linear(hs * 2, cls_num),
            )
            self.chestXray_classifier.apply(objectives.init_weights)  
        ####################################
        ...
        self.lora_layers = self.hparams.config["lora_layers"]

        lora_list = [None  for i in range(len(self.transformer.blocks))]
        for i in self.lora_layers:
            lora_list[i] = LoRAMissingLayer(in_features=embed_dim, out_features=embed_dim, rank=4, network_alpha=None, device=self.device, dtype=self.dtype)
    ...
    def infer(
        ...
        prompts = None
        for idx in range(len(batch["missing_type"])):
            if batch["missing_type"][idx] == 0:
                ...
                # W = torch.nn.functional.softmax(self.W_complete, dim=-1)
                # prompt = torch.bmm(W, self.basic_prompt)    
                ...
                prompt = prompt.unsqueeze(0)
                #prompt = self.complete_prompt 
                # W = torch.nn.functional.softmax(self.W_missing_text, dim=-1) 
                # prompt = torch.bmm(W, self.basic_prompt)    
            elif batch["missing_type"][idx] == 2:
                prompt = self.missing_img_prompt
                prompt = prompt.unsqueeze(0)
                #prompt = self.complete_prompt 
                # W = torch.nn.functional.softmax(self.W_missing_img, dim=-1)  
                # prompt = torch.bmm(W, self.basic_prompt)    
        ...
        for i, blk in enumerate(self.transformer.blocks):
            ...
            if i in self.lora_layers:
                ...
                lora_tensor = []
                flag = 0
                for b in range(len(batch["missing_type"])):
                    temp = x[flag:flag+1]
                    temp_lora = self.lora_list[i](temp,batch["missing_type"][b])
...
#         cls_feats = self.pooler(x[:,:prompts.size(1)].mean(dim=1,keepdim=True))
    ...
    def forward(self, batch):
        ...
        if "chestXray" in self.current_tasks:
            ret.update(objectives.compute_chestXray(self, batch))               
