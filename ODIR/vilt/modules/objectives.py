...
def compute_ODIR(pl_module, batch):
    phase = "train" if pl_module.training else "val"
    if phase == "train":
        infer = pl_module.infer(batch, mask_text=False, mask_image=False)
    else:
    ...
    imgcls_logits = pl_module.ODIR_classifier(infer["cls_feats"])
    ...
    ret = {
        "ODIR_loss": imgcls_loss,
        "ODIR_logits": imgcls_logits,
        "ODIR_labels": imgcls_labels,
    }

    loss = getattr(pl_module, f"{phase}_ODIR_loss")(ret["ODIR_loss"])
    
    f1_scores = getattr(pl_module, f"{phase}_ODIR_F1_scores")(
        ret["ODIR_logits"], ret["ODIR_labels"]
    )
    pl_module.log(f"ODIR/{phase}/loss", loss)
