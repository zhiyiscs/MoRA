...
class ViLTransformerSS(pl.LightningModule):
    def __init__(self, config):
        ...
        ##################################
        if self.hparams.config["loss_names"]["ODIR"] > 0:
            cls_num = self.hparams.config["ODIR_class_num"]
            self.ODIR_classifier = nn.Sequential(
                nn.Linear(hs, hs * 2),
                nn.LayerNorm(hs * 2),
                nn.GELU(),
                nn.Linear(hs * 2, cls_num),
            )
            self.ODIR_classifier.apply(objectives.init_weights)  
        ####################################
...
#         cls_feats = self.pooler(x[:,:prompts.size(1)].mean(dim=1,keepdim=True))
    ...
    def forward(self, batch):
        ...
        if "ODIR" in self.current_tasks:
            ret.update(objectives.compute_ODIR(self, batch))               
