...
def config():
    ...
    missing_ratio = {'train': rate, 'val': rate, 'test': rate}
    missing_type = {'train': type, 'val': type, 'test': type} # ['text', 'image', 'both'] in VL taskss
...
###########################################
@ex.named_config
def task_finetune_ODIR():
    exp_name = "finetune_ODIR"
    datasets = ["ODIR"]
    loss_names = _loss_names({"ODIR": 1})
    batch_size = 64
    max_epoch = 100
    max_steps = None
    warmup_steps = 0.02
