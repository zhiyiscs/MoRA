...
class BaseDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        self.training_table = pa.ipc.RecordBatchFileReader(
                    pa.memory_map(f"{data_dir}/ODIR_train.arrow", "r")
                ).read_all()
    ...
    def get_raw_image(self, index, image_key="image"):
        ...
        image_bytes = io.BytesIO(self.table['left_image'][index].as_py())
        image_bytes.seek(0)
        left_image = Image.open(image_bytes).convert("RGB")

        image_bytes = io.BytesIO(self.table['right_image'][index].as_py())
        image_bytes.seek(0)
        right_image = Image.open(image_bytes).convert("RGB")
    ...
    def get_new_text(self, raw_index):

        text = str(self.training_table["text"][self.text[raw_index]])
        encoding = self.tokenizer(
            text,
            padding="max_length",
            truncation=True,
    ...
    def collate(self, batch, mlm_collator):
        ...
        missing_type =  dict_batch["missing_type"]
        ...
        if len(txt_keys) != 0:
            ...
            for i, txt_key in enumerate(txt_keys):
                ...
                if "train" in self.names[0]:
                    input_ids = torch.zeros(len(encodings),mlm_ids.size(1)).long()
                    attention_mask = torch.zeros((len(encodings),mlm_ids.size(1))).long()
                else:
